...
void ASGObstacleBase::PlaySpawnSound()
{
	if (!HasAuthority()) return;
	
	MulticastPlaySpawnSound(GetActorLocation());
}
...
void ASGObstacleBase::MulticastPlaySpawnSound_Implementation(FVector SoundLocation)
{
	if (GetNetMode() == NM_DedicatedServer || SpawnSound == nullptr) return;
	
	UGameplayStatics::PlaySoundAtLocation(this, SpawnSound, SoundLocation);
}
